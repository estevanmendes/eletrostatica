...
class poisson_equation:
    def __init__(self,x_length,y_length,realism=False):
        ...
        if realism:
            ...
            self.CHARGE_GRID=np.zeros((x_length+x_length*grid_increase,y_length+y_length*grid_increase))
        else:            
            self.SQUARE_GRID=np.zeros((x_length+2,y_length+2))
            #the grid is inside other larger grid because the Contour
            self.SQUARE_GRID=np.zeros((x_length+2,y_length+2))
        
    def contour_rectangle_capacitor(self,x_bottom,x_top,y_bottom,y_top):
    ...
    def charge_space(self,number_charges,x_positions,y_positions,charge_density_value):
        CHARGE_INPUT=np.zeros((self.x_length,self.y_length))
        for i in range(number_charges):
                CHARGE_INPUT[x_positions[i],y_positions[i]]=charge_density_value[i]
        if self.realism:
            X_lim_inf=int(self.x_length*self.grid_increase/2)-1
            X_lim_sup=int(self.x_length*self.grid_increase/2)-1+self.x_length
            Y_lim_inf=int(self.y_length*self.grid_increase/2)-1
            Y_lim_sup=int(self.y_length*self.grid_increase/2)-1+self.y_length
            self.CHARGE_GRID[X_lim_inf:X_lim_sup,Y_lim_inf:Y_lim_sup]=CHARGE_INPUT
        else:
            self.CHARGE_GRID[1:-1,1:-1]=CHARGE_INPUT
    ...
    def iterate_rounds(self,rounds,vector1):
        x_length=self.x_length
        y_length=self.y_length
        contour_nonzero=np.nonzero(vector1)
        ...
        for z in range(0,rounds):
            ...
            for k in range(x_length*y_length):                                
                ...
                vector1[i,j]=np.pi*self.CHARGE_GRID[i,j]+0.25*(GRID[i+1,j]+GRID[i-1,j]+GRID[i,j-1]+GRID[i,j+1])
            vector1[contour_nonzero]=self.SQUARE_GRID[contour_nonzero]
    ...
    def iterate_precision(self,precision,vector1):
        ...
        iteration=0
        while(iteration<10 or np.fabs(np.trace(vector1)-np.trace(GRID))>precision): 
            GRID=np.array(vector1)  
            for k in range(x_length*y_length):                
                i=k%x_length+1
                j=int(k/x_length)+1
